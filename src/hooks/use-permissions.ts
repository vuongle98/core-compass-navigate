...
export function usePermissions(): UserPermissions {
  ...
  return useMemo(() => {
    ...
    if (isAdmin) {
      ...
      const hasFeatureFlags = uniquePermissions.some(
        (p) =>
    ...
    const result: UserPermissions = {
      ...
      hasRole: (roleCode: string) =>
        roles.some(
          (role) => role.code?.toUpperCase() === roleCode.toUpperCase()
        ),
    };
...
export function useGuard(requiredPermission: string | Permission): {
  allowed: boolean;
} {
