      ...
      {!readOnly && (
        <div className="flex flex-wrap gap-1 p-2 border-b bg-muted/50">
          <div className="flex gap-1 mr-1 border-r pr-1">
            ...
            <ToolbarButton icon={Italic} onClick={formatItalic} tooltip="Italic" />
            <ToolbarButton icon={Underline} onClick={formatUnderline} tooltip="Underline" />
          </div>
          
          <div className="flex gap-1 mr-1 border-r pr-1">
            <ToolbarButton icon={AlignLeft} onClick={alignLeft} tooltip="Align Left" />
            <ToolbarButton icon={AlignCenter} onClick={alignCenter} tooltip="Align Center" />
            <ToolbarButton icon={AlignRight} onClick={alignRight} tooltip="Align Right" />
          </div>
          
          <div className="flex gap-1 mr-1 border-r pr-1">
            <ToolbarButton icon={Heading1} onClick={formatH1} tooltip="Heading 1" />
            <ToolbarButton icon={Heading2} onClick={formatH2} tooltip="Heading 2" />
            <ToolbarButton icon={Heading3} onClick={formatH3} tooltip="Heading 3" />
          </div>
          
          <div className="flex gap-1 mr-1 border-r pr-1">
            <ToolbarButton icon={List} onClick={insertUnorderedList} tooltip="Bullet List" />
            <ToolbarButton icon={ListOrdered} onClick={insertOrderedList} tooltip="Number List" />
          </div>
          
          <div className="flex gap-1 mr-1 border-r pr-1">
            <ToolbarButton icon={Quote} onClick={formatQuote} tooltip="Quote" />
            <ToolbarButton icon={Code} onClick={insertCode} tooltip="Code Block" />
            <ToolbarButton icon={LinkIcon} onClick={insertLink} tooltip="Insert Link" />
            {allowImageUpload && (
              <ToolbarButton icon={ImageIcon} onClick={triggerImageUpload} tooltip="Insert Image" />
            )}
          </div>
          ...
          <div className="flex gap-1">
            ...
            <ToolbarButton icon={Trash2} onClick={clearFormatting} tooltip="Clear Formatting" />
          </div>
        </div>
      )}
      ...
      {!readOnly && (
        <div className="border-t p-2 text-xs text-muted-foreground flex justify-between items-center">
          ...
          <div className="text-right">
            ...
            &nbsp;•&nbsp;
            <kbd className="px-1 py-0.5 rounded bg-muted">Shift+Tab</kbd> to outdent
          </div>
        </div>
      )}
    </div>
