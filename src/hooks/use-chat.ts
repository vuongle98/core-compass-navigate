...
export function useChat(): UseChatReturn {
  ...
  useEffect(() => {
    if (activeChat) {
      ...
      LoggingService.info(
        "chat",
        "messages_loaded",
        `Loaded ${chatMessages.length} messages for chat ${activeChat.id}`
      );
    } else {
      setMessages([]);
    }
  ...
  const handleNewMessage = (message: Message) => {
    if (activeChat && activeChat.id === message.chatId) {
      ...
      LoggingService.info(
        ...
        "New message received for active chat"
      );
    }
  };
  ...
  const handleChatsUpdated = (updatedChats: Chat[]) => {
    ...
    LoggingService.info(
      "chat",
      "chats_updated",
      `Chats list updated, now has ${updatedChats.length} chats`
    );
  };
  ...
  const handleChatCreated = (chat: Chat) => {
    ...
    LoggingService.info(
      "chat",
      "chat_created_event",
      `New chat created: ${chat.name || chat.id}`
    );
  };
  ...
  const sendMessage = (
    ...
    LoggingService.info(
      "chat",
      "message_sent",
      `Message sent to chat ${activeChat.id}`
    );
  };
  ...
  const createPrivateChat = (
    ...
    LoggingService.info(
      ...
      `Created private chat with user ${userName}`
    );
  };
