...
const makeRequest = async <T>(
  ...
  const response: AxiosResponse<ApiResponse<T>> = await apiClient(url, options);
  const duration = Date.now() - startTime;

  // Log response time for performance monitoring
  console.log(
    `API ${options.method || "GET"} ${url} - ${duration}ms - Status: ${
      response.status
    }`
  );
...
const getPaginated = async <T>(
  ...
  try {
    ...
    // Make the request
    const response = await makeRequest<PaginatedData<T>>(
