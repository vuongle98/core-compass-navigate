...
import { 
  BarChart, Settings, Users, Shield, Key, FileText, Bell, 
  Blocks, Flag, ScrollText, ClipboardList, Wrench, Bot, 
  FileBox, BookOpen, Cog, ChevronDown, ChevronRight,
  ListTodo, Hexagon, Server, LayoutList
} from 'lucide-react';
...
const NavItem = ({ href, icon: Icon, title, badge, collapsed, onClick }: NavItemProps) => {
        ...
        <div className="group relative">
          <div className="absolute left-full top-0 ml-1 w-48 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
            <div className="mt-0 py-2 bg-sidebar rounded-md shadow-lg border border-sidebar-border overflow-hidden">
              <div className="px-3 py-2 font-medium text-sm border-b border-sidebar-border">{title}</div>
              {children}
            </div>
          </div>
        </div>
